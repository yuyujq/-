...
public interface UsersDao {
    ...
    @Select("select * from users where username like concat('%',#{username},'%') order by id desc limit #{begin}, #{size} ")//order by id desc 
	public List<Users> getListByName(@Param("username")String username, @Param("begin")int begin, @Param("size")int size);
    @Select("select count(*) from users where username like concat('%',#{username},'%')")
	public long getTotalByName(@Param("username")String username);
