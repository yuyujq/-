...
public class OrderService {
	 ...
	 * @return
	 */
	public List<Orders> getListVgoods(byte status, int page, int row) {
		List<Orders> orderList = orderDao.getListByStatusVgoods(status, row * (page-1), row);
	 ...
	 * @return
	 */
	public List<Orders> gett(int id){
        List<Orders> list = (List<Orders>) Arrays.asList(orderDao.getListById(id));
		return list;
	}
	...
	public List<Orders> getList1(int page, int row) {
	 ...
	 * @return
	 */
	public boolean confrim(int id) {
		Orders order = orderDao.selectById(id);
		order.setStatus(Orders.STATUS_CONFRIM);
		return orderDao.updateByIdSelective(order) > 0;
	}
