...
public class GoodService {
	...
	public List<Goods> getList(int status, int page, int size,int lprice,int hprice){
		if (status == 0) {
			return packTopList(goodDao.getList(size * (page-1), size));
		}
		List<Tops> topList = topService.getList((byte)status, page, size);
		if(topList!=null && !topList.isEmpty()) {
			List<Goods> goodList = new ArrayList<>();
			for(Tops top : topList) {
				if(goodDao.selectById1(top.getGoodId(),lprice,hprice)!=null) {
				goodList.add(packTop(goodDao.selectById1(top.getGoodId(),lprice,hprice)));}
			}
			return goodList;
		}
		return null;
	...
	public List<Goods> getListByType(int typeid, int page, int size,int lprice,int hprice) {
		return typeid > 0 ? goodDao.getListByType1(typeid, size * (page-1), size,lprice,hprice) : goodDao.getList(size * (page-1), size);
	}
	...
	public long getTotalByType(int typeid,int lprice,int hprice){
		return typeid > 0 ? goodDao.getTotalByType1(typeid,lprice,hprice) : goodDao.getTotal();
	}
